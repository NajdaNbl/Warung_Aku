            ...
            <div class="space-y-3 sm:space-y-4">
                ...
                @foreach($cart as $id => $item)
                    @php
                        ...
                        $grandTotal += $subtotal;
                    @endphp
                    <div class="bg-white rounded-2xl p-3 sm:p-6 shadow-sm border border-gray-100">
                        <div class="flex gap-3 sm:gap-4">
                            <div class="w-20 sm:w-24 h-20 sm:h-24 rounded-xl overflow-hidden bg-gray-100 shrink-0">
                                <img src="{{ $item['image'] ?: asset('images/products/placeholder.svg') }}" alt="{{ $item['name'] }}" class="w-full h-full object-cover">
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex justify-between items-start gap-2">
                                    <div class="min-w-0">
                                        <h3 class="font-semibold text-[#1B4332] text-sm sm:text-base truncate">{{ $item['name'] }}</h3>
                                        <p class="text-[#D4A373] font-bold text-sm sm:text-base mt-0.5">{{ 'Rp' . number_format($item['price'], 0, ',', '.') }}</p>
                                    </div>
                                    <div class="text-right shrink-0 ml-2">
                                        <p class="text-xs text-gray-400 hidden sm:block">Subtotal</p>
                                        <p class="font-bold text-[#1B4332] text-sm sm:text-base">{{ 'Rp' . number_format($subtotal, 0, ',', '.') }}</p>
                                    </div>
                                </div>
                                <div class="flex items-center justify-between mt-2 sm:mt-3">
                                    <form action="{{ route('cart.update', $id) }}" method="POST" class="flex items-center">
                                        ...
                                        <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                                            <button type="button" onclick="this.parentNode.querySelector('input').stepDown(); this.closest('form').submit();" class="px-2.5 sm:px-3 py-2 sm:py-1.5 hover:bg-gray-50 text-gray-500 text-sm">-</button>
                                            <input type="number" name="quantity" value="{{ $item['quantity'] }}" min="1" max="{{ $item['stock'] }}" class="w-10 sm:w-12 text-center py-2 sm:py-1.5 border-x border-gray-200 outline-none text-sm" readonly>
                                            <button type="button" onclick="this.parentNode.querySelector('input').stepUp(); this.closest('form').submit();" class="px-2.5 sm:px-3 py-2 sm:py-1.5 hover:bg-gray-50 text-gray-500 text-sm">+</button>
                                        </div>
                                    </form>
                        ...
                        <div class="sm:hidden flex justify-between items-center mt-2 pt-2 border-t border-gray-100">
                            <span class="text-xs text-gray-400">Subtotal</span>
                            <span class="font-bold text-[#1B4332] text-sm">{{ 'Rp' . number_format($subtotal, 0, ',', '.') }}</span>
                        </div>
                    </div>
                @endforeach
            </div>

            <div class="mt-6 sm:mt-8 bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
                <div class="flex items-center justify-between mb-3 sm:mb-4">
                    <span class="text-sm sm:text-base text-gray-500">Total Item:</span>
                    <span class="font-semibold text-[#1B4332] text-sm sm:text-base">{{ array_sum(array_column($cart, 'quantity')) }} item</span>
                </div>
                <div class="flex items-center justify-between mb-4 sm:mb-6">
                    <span class="text-base sm:text-lg font-semibold text-gray-700">Total Harga:</span>
                    <span class="text-xl sm:text-2xl font-bold text-[#D4A373]">{{ 'Rp' . number_format($grandTotal, 0, ',', '.') }}</span>
                </div>

                <div class="border-t border-gray-100 pt-3 sm:pt-4 mb-3 sm:mb-4 space-y-3">
                ...
                <form id="checkoutForm" action="{{ route('checkout.process') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full py-4 bg-[#1B4332] text-white font-bold rounded-xl hover:bg-[#2D6A4F] transition-all duration-300 flex items-center justify-center space-x-2 text-base sm:text-lg">
                        <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z" /></svg>
